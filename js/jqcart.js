...
;(function($) {
  ...
  var cartData,
    ...
    totalCnt = 0,
    visibleLabel = false,
    label = $('<div class="jqcart-cart-label"><span class="jqcart-title">YOUR BAG</span><span class="jqcart-total-cnt">0</span></div>'),
    modal = '<div class="jqcart-layout"><div class="jqcart-checkout">123</div></div>',
    orderform = '<form class="jqcart-orderform" method="post"><div class="inputBoxKor"><input type="text" class="from-control" name="login" id="login" required=""><label>Username</label><div class="inputBoxKor"><input type="password" class="from-control" name="pass" id="pass" required=""><label>Password</label><input type="submit" value="Pay Order"><input type="reset" value="Back to Shopping"></p></form>';
  var opts = {
		buttons: '.add_item',
		cartLabel: 'body',
		visibleLabel: false,
  ...
  var actions = {
    ...
    openCart: function() {
      var subtotal = 0,
			cartHtml = '';
      cartData = actions.getStorage();
      orderPreview = '<p class="jqcart-cart-title">Your Bag <span class="jqcart-print-order"></span></p><div class="jqcart-table-wrapper"><div class="jqcart-manage-order"><div class="jqcart-thead"><div>ID</div><div>Name of product</div><div>Price</div><div>Amount of goods</div><div>Sum total</div><div></div></div>';
      var key, sum = 0;
      for (key in cartData) {
        if (cartData.hasOwnProperty(key)) {
      ...
      orderPreview += '<div class="jqcart-subtotal">Subtotal: <strong>' + subtotal + '</strong> ' + opts.currency + '</div>';
			
			cartHtml = subtotal ? (orderPreview + orderform) : '<h2 class="jqcart-empty-cart">Your bag is empty</h2>';
    ...
    sendOrder: function(e) {
      e.preventDefault();
      var $that = $(this);
      if ($.trim($('[name=login]', $that).val()) === '' || $.trim($('[name=pass]', $that).val()) === '') {
        $('<p class="jqcart-error">Please enter your Login and Password.</p>').insertBefore($that).delay(3000).fadeOut();
        return false;
      }
      $.ajax({
        url: opts.handler,
